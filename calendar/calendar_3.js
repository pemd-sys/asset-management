...
// This ensures the text (white or black) always remains readable regardless of the background color chosen for the pill
...
async function fetchAndRender() {
  ...
  for (let d = new Date(startGrid); d <= endGrid; d.setDate(d.getDate() + 1)) {
    ...
    for (let i=0; i < Math.min(dayEvents.length, MAX_EVENT_ROWS); i++){
      ...
      pill.classList.add("mycal-ev");  // base style stays the same

      // apply event color (any valid CSS color)
      if (ev.color) {
        pill.style.backgroundColor = ev.color.trim();
        // adjust text color automatically for contrast (optional)
        if (isDarkColor(ev.color)) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    const action = formData.get("id") ? "update_event" : "add_event";

    try {
      const res = await fetch(`calendar_3.php?action=${action}`, {
        method: "POST",
        body: formData
      });
