...
// Get filters from URL parameters with validation
$filters = [];
if (isset($_GET['brand']) && is_array($_GET['brand'])) {
    $filters['brand'] = array_map('trim', $_GET['brand']);
}
if (isset($_GET['bandwidth']) && is_array($_GET['bandwidth'])) {
    $filters['bandwidth'] = array_map('trim', $_GET['bandwidth']);
}
...
if (isset($_GET['sort'])) {
    $allowedSorts = ['price_low', 'price_high', 'brand', 'popular'];
    $filters['sort'] = in_array($_GET['sort'], $allowedSorts) ? $_GET['sort'] : 'price_low';
}
...
// Get current bookings using the same database connection
$stmt = $db->prepare("
